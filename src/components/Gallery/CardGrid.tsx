...
export const CardGrid = memo(function CardGrid({ cards, onCardClick }: CardGridProps) {
  ...
  const handleScroll = useCallback(() => {
    ...
    const end = Math.min(lastRow * cols + cols * BOTTOM_BUFFER_ROWS, visibleCount);
    ...
    if (isDesktop) {
      for (let i = 0; i < Math.min(visibleCount, cols * BOTTOM_BUFFER_ROWS); i++) {
        if (cards[i]) newActive.add(cards[i].id);
      }
    }
    ...
    <div className={isDesktop ? 'columns-4 gap-3 px-4 pb-20' : 'gallery-grid grid gap-3 px-4 pb-20'}
      ...
      {visibleCards.map((card) => (
        <div key={card.id} onClick={() => onCardClick(card)} className="mb-3 break-inside-avoid">
          <CardItem card={card} isActive={isDesktop || activeIds.has(card.id)} isDesktop={isDesktop} />
        </div>
      ))}
    </div>
  );
